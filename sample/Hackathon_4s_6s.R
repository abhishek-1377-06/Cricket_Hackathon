...
overall_batsman_stats <- data.frame(matrix(nrow=0, ncol=12)) 
colnames(overall_batsman_stats) <- c('teamName','opposition','player_name','runs',
                                     '4s','6s','balls_faced','strike_rate','match_format','city','venue','date')
# add innings number and position
...
for(input_file in file_names){
  ...
  if(grepl('yaml', input_file)){
    ...
    for(innings in df$innings){
      ...
      runs_info$city <- if_else(is.null(general_match_stats$city),as.factor('Not Available'),general_match_stats$city)
      ...
      colnames(batsman_stats) <- c('teamName','opposition','player_name','match_format','city','venue','date','runs','4s','6s','balls_faced','strike_rate')
